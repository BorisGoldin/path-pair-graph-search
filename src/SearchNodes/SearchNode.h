...
public:
	SearchNode() = default;
    SearchNode(const Idx vertex_id, SearchNodePtr parent, Pair<CostType> cost_until_now, Pair<CostType> future_heuristic_cost={0,0});
    SearchNode(const Idx vertex_id, SearchNodePtr parent, Triplet<CostType> cost_until_now, Triplet<CostType> future_heuristic_cost={0,0,0});
    ...
    bool is_dominated_by(const SearchNodePtr& other, Triplet<double> eps, bool use_heuristic) const;

    static SearchNodePtr extend(const SearchNodePtr& node, const Edge& edge, Pair<CostType> future_heuristic_cost);
