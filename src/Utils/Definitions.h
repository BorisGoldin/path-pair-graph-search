...
{
private:
    std::map<Idx, std::vector<Edge>> matrix; // Idx is Vertex.id
public:
    AdjacencyMatrix() = default;
    AdjacencyMatrix(std::vector<Edge>& edges, bool inverse=false);
    void add(Edge edge);
    size_t get_number_of_vertices() const;
    std::vector<Edge>& operator[](Idx vertex_id); 
    const std::vector<Edge>& operator[](Idx vertex_id) const; 
